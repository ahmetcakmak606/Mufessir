...
export async function calculateEmbeddingSimilarity(
  ...
  // Check if OpenAI is available (even in sample mode if API key is set)
  const openai = process.env.OPENAI_API_KEY
    ? (await import("openai")).default
    : null;

  // If no OpenAI API key, fall back to text similarity
  if (!openai) {
    console.warn("No OpenAI API key, falling back to text similarity");
  ...
  const clip = (s: string) =>
    s.length > MAX_EMBED_INPUT_CHARS ? s.slice(0, MAX_EMBED_INPUT_CHARS) : s;
  ...
  try {
    ...
    const embedding1 = emb1.data?.[0]?.embedding;
    const embedding2 = emb2.data?.[0]?.embedding;

    if (!embedding1 || !embedding2) {
      console.warn("No embedding returned, falling back to text similarity");
      return calculateTextSimilarity(aiResponse, tafsirText);
    }
...
export async function findMostSimilarTafsir(
  ...
  for (const tafsir of tafsirs) {
    try {
      ...
      console.warn(
        `Error calculating similarity for tafsir ${tafsir.tafsirId}:`,
        error,
      );
    }
  }

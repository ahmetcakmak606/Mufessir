...
async function main() {
  const sqlitePath =
    ...
    resolve(__dirname, "../../../mufessir_ai_database.db");
  ...
  for (const r of scholarRows) {
    ...
    const deathYear = Number.isFinite(Number(r.death_miladi))
      ? Number(r.death_miladi)
      : null;
    await prisma.scholar.upsert({
      ...
      create: {
        id,
        name:
          asString(r.mufassir_tr) ||
          asString(r.mufassir_en) ||
          `Müfessir ${r.mufassirID}`,
        birthYear: null,
        deathYear,
        century: r.century ?? 0,
  ...
  for (const s of surahRows)
    surahNameById.set(
      s.SurahID,
      asString(s.surah_name) ||
        asString(s.surah_name_ar) ||
        `Surah ${s.SurahID}`,
    );
  ...
  if (process.env.IMPORT_TAFSIRS === "true") {
    ...
    const bookRows: Row[] = db
      .prepare("SELECT bookID, page_id, text_content FROM tafsir_books")
      .all();
    ...
    for (const br of bookRows) {
      ...
      bookTextById.set(
        br.bookID,
        existing + (asString(br.text_content) || "") + "\n",
      );
    }
    ...
    for (const t of tafsirRows) {
      ...
      const tafsirText =
        asString(bookTextById.get(t.BookID)) || asString(t.Description) || "";
    ...
    console.log(
      "Skipping tafsirs import (set IMPORT_TAFSIRS=true to include).",
    );
  }

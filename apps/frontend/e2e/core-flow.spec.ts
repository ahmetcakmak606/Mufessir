  ...
  await page.route("**/mock-api/**", async (route) => {
    ...
    if (pathname === `/tafseer/runs/${run.runId}` && method === "PATCH") {
      ...
      if (Object.prototype.hasOwnProperty.call(payload, "title"))
        run.title = payload.title || null;
      if (Object.prototype.hasOwnProperty.call(payload, "notes"))
        run.notes = payload.notes || null;
      if (
        Object.prototype.hasOwnProperty.call(payload, "starred") &&
...
test.describe("Dashboard Core Flow", () => {
  test("auth -> query -> save -> history -> replay", async ({
    page,
  }, testInfo) => {
    ...
    await expect(
      page.getByText("Mercy-centered interpretation."),
    ).toBeVisible();
    ...
    await expect
      .poll(() => mockApi.getStreamCallCount())

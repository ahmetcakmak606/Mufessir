...
function sortRuns(items: RunSummary[], sortBy: RunsSort): RunSummary[] {
  ...
  if (sortBy === "citations") {
    next.sort(
      (a, b) =>
        b.citationsCount - a.citationsCount ||
...
export function filterAndSortRuns(
  ...
  const filtered = runs.filter((run) => {
    ...
    if (
      ...
      run.provenance !== state.provenanceFilter
    )
      return false;
    if (state.citationFilter === "WITH" && run.citationsCount <= 0)
      return false;
    if (state.citationFilter === "WITHOUT" && run.citationsCount > 0)
      return false;
    if (!normalizedQuery) return true;
    const verseRef = `${run.verse.surahNumber}:${run.verse.verseNumber}`;
    const haystack =
      `${run.title || ""} ${run.verse.surahName} ${verseRef} ${run.aiResponsePreview || ""}`.toLowerCase();

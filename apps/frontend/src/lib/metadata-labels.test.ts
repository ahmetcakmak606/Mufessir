...
describe("metadata labels", () => {
  it("maps facet values to localized labels", () => {
    expect(formatFacetValue("tr", "periodCodes", "FOUNDATION")).toBe(
      "Kurucu Dönem",
    );
    expect(
      formatFacetValue("en", "sourceAccessibilities", "FULL_DIGITAL"),
    ).toBe("Fully Digital");
  });

  it("falls back to original value when unknown", () => {
    expect(formatFacetValue("en", "madhabs", "UnknownMadhab")).toBe(
      "UnknownMadhab",
    );
  });

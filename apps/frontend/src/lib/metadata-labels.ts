...
const FACET_VALUE_LABELS: Record<
  UiLang,
  Record<FacetFilterKey, Record<string, string>>
> = {
  tr: {
    periodCodes: {
...
export function formatScholarName(name: string): string {
  return name
    .toLowerCase()
    .replace(/(?:^|\s)(al-|ibn|abd|bin)\b/gi, (match) => match.toLowerCase())

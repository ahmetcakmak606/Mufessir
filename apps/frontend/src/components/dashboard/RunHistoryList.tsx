    ...
    <div className="space-y-3">
      {runs.map((run) => (
        <article
          ...
          className="ui-panel-strong overflow-hidden"
        >
          <div className="space-y-3 px-4 py-4 sm:px-5 sm:py-5">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {run.starred && (
                    <span className="ui-badge">{labels.starredBadge}</span>
                  )}
                </div>
                <p className="ui-muted text-xs">
                  {labels.createdAt}: {new Date(run.createdAt).toLocaleString()}{" "}
                  · {labels.updatedAt}:{" "}
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              ...
              <span className="ui-badge">
                {labels.citations}: {run.citationsCount}
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Link
                href={`/dashboard/query?runId=${run.runId}`}
                data-testid={`edit-run-${run.runId}`}
                className="ui-button-secondary px-3 py-1.5 text-xs"
              >
                {labels.editInputs}
              </Link>
              <Link
                ...
                data-testid={`replay-run-${run.runId}`}
